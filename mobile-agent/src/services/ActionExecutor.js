/**
 * ActionExecutor - 执行设备操作 (v2.0)
 * 新增：剪贴板、文件、通知、应用、网络、音频、传感器、远程控制、任务计划
 */
...
export class ActionExecutor {
  ...
  async capturePhoto(payload) {
    return { success: false, note: '请在App界面手动拍照' };
  }
  ...
  async getNetworkInfo(payload) {
    ...
    return {
      ...
      details: state.details ? {
        ...
        cellularGeneration: state.details.cellularGeneration, // '2g', '3g', '4g', '5g'
  ...
  async recordAudio(payload) {
    const { duration = 30, filename = `recording_${Date.now()}.aac` } = payload;
    const path = `${AudioUtils.DocumentDirectoryPath}/${filename}`;
    
    try {
      await AudioRecorder.requestAuthorization();
      
      const audioPath = await AudioRecorder.prepareRecordingAtPath(path, {
        SampleRate: 22050,
        Channels: 1,
        AudioQuality: 'High',
        AudioEncoding: 'aac',
      ...
      this.activeRecordings.set(filename, { path, startTime: Date.now() });
  ...
  async playAudio(payload) {
    ...
    return new Promise((resolve, reject) => {
      const sound = new Sound(path, '', (error) => {
        ...
        sound.play((success) => {
          if (success) {
            resolve({ success: true, duration: sound.getDuration() });
          } else {
  ...
  async stopAudio(payload) {
    ...
    try {
      ...
      if (recording) {
        ...
        return { success: true, path: recording.path, duration };
      }
      
      return { success: true, note: '没有正在进行的录音' };
    } catch (error) {
      return { success: false, error: error.message };
    }
  }
  ...
  async scanQRCode(payload) {
    return {
      success: false,
      note: '二维码扫描需要安装 react-native-camera 并打开相机界面',
    };
  }
  ...
  async controlDevice(payload) {
    ...
    try {
      switch (action) {
        ...
        case 'volume':
          // value: 0-1, type: 'music' | 'system' | 'call'
  ...
  async scheduleTask(payload) {
    ...
    try {
      const jobConfig = {
        ...
        notificationTitle: 'Mobile Agent 任务',
        notificationText: '正在执行计划任务...',
      };
      ...
      return {
        ...
        taskId,
        scheduled: true,
        nextRun: new Date(Date.now() + delay * 1000).toISOString(),
      };
    } catch (error) {
      return { success: false, error: error.message };
    }
  ...
  async cancelTask(payload) {
    ...
    try {
      await BackgroundJob.cancel({ jobKey: taskId });
